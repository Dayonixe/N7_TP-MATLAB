...
% Definition de la série statistique 2D :
I_gauche = I(:,1:end-1);
I_gauche = I_gauche(:);
I_droite = I(:,2:end);
I_droite = I_droite(:);
...
variance_abscisses = mean(I_gauche.^2) - (moyenne_abscisses)^2;
variance_ordonnees = mean(I_droite.^2) - (moyenne_ordonnees)^2;
covariance = mean(I_droite .* I_gauche) - moyenne_abscisses * moyenne_ordonnees;
